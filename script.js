...
function GameController(
  ...
  const checkWinner = () => {
    ...
    const antiDiagonalWin = currentBoard.every(
      (row, index) => row[row.length - index - 1].getValue() === playerToken,
    );
  ...
  const updateScreen = () => {
    ...
    board.forEach((row, rowIndex) => {
      row.forEach((cell, colIndex) => {
        ...
        cellToken.textContent = cell.getValue() === 0 ? "" : cell.getValue();

        if (cellToken.textContent === "O") {
          cellToken.style.setProperty("color", "var(--color-playerOne)");
        } else if (cellToken.textContent === "X") {
          cellToken.style.setProperty("color", "var(--color-playerTwo)");
        }
